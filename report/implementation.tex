...
\begin{task}
	\taskschedule{Wk1 February}
	\taskrisks{None - useful to have early on, but will not block progress.}
\end{task}
...
\begin{task}
	\taskschedule{Wk4 February}
	\taskrisks{Time-sensitive - any delay blocks further progress; bad choice impacts rest of project.}
\end{task}
...
\begin{task}
	\taskschedule{Wk2 March}
	\taskrisks{Needed, at least in part, early on to avoid surprises later.}	
\end{task}
...
\begin{task}
	\taskschedule{Wk2 April}
	\taskrisks{Crux of the project, and entirely dependent on~\ref{task-eval-mir-cfg}.}
\end{task}
...
\begin{task}
	\taskschedule{Wk1 June\footnotemark}
	\taskrisks{Overrunning would encroach on time available to analyse and write-up results.}
\end{task}\footnotetext{The prolonged gap between milestones takes cognisance of the exam period enclosed.}
